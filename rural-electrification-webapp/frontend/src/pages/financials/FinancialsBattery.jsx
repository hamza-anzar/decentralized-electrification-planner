...
const SYSTEM_SCALED = new Set(["Solar Panels (590 W rated)", "Battery System"]);
const blankDash = (v) => (v === null || v === undefined || v === "" ? "—" : v);

export default function FinancialsBattery({ color }) {
  const { data: defaults, error: loadError } = useApiGet("/api/financials/defaults");
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      api
        .post("/api/financials/compute", { boqItems, landCostOptions, costParameters })
        .then((r) => {
          setResult(r);
          setError(null);
        })
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      const r = await api.post("/api/financials/save", { boqItems, landCostOptions, costParameters });
      setResult(r);
      setSaved(true);
      refetchProgress();
    } catch (e) {
      ...
      {!ready ? (
        ...
        <>
          <div className="flex items-center justify-between">
            <p className="text-xs text-ink-400">EUR is this project's base currency (converted once from the original PKR workbook). PKR/USD below are display-only.</p>
          ...
          <Card>
            <SectionHeader
              ...
              subtitle="Costs in EUR. Standard rows: qty x unit cost. Solar Panels & Battery System: system size (from Solar Design) x lumpsum rate — edit that rate directly on the row."
              color={color}
            />
            <div className="mb-4">
              <FileUpload
                downloadPath="/api/financials/template"
                downloadFilename="Cost_BOQ_Template.xlsx"
                uploadPath="/api/financials/upload"
          ...
          <Card>
            <SectionHeader icon={MapPinned} title="Land Cost Options" subtitle="Which approach is actually used is chosen by the land_cost_approach parameter below — shared with the Solar+Diesel scenario" color={color} />
            <EditableTable
              columns={[
