...
const SYSTEM_SCALED = new Set(["Solar Panels (590 W rated)", "Diesel Generator(s)", "Generator Installation & Commissioning"]);
const blankDash = (v) => (v === null || v === undefined || v === "" ? "—" : v);

export default function FinancialsDiesel({ color }) {
  const { data: defaults, error: loadError } = useApiGet("/api/financials-diesel/defaults");
  ...
  useEffect(() => {
    if (!boqItems || !costParameters) return;
    setComputing(true);
    const handle = setTimeout(() => {
      api
        .post("/api/financials-diesel/compute", { boqItems, costParameters })
        .then((r) => {
          setResult(r);
          setError(null);
        })
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      const r = await api.post("/api/financials-diesel/save", { boqItems, costParameters });
      setResult(r);
      setSaved(true);
      refetchProgress();
    } catch (e) {
  ...
  const ready = boqItems && costParameters;
      ...
      {!ready ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <KpiCard icon={Receipt} label="Total Opex (Lifetime, incl. fuel)" value={result ? conv(result.opex.total_opex_eur) : null} compact suffix={` ${currency}`} color="#7a5cd6" />
          ...
          <Card>
            <SectionHeader
              ...
              subtitle="The same PV array costs as Solar+Battery (panels, inverter, cabling, civil works, etc.) minus the battery, plus the generator and its installation. Standard rows: qty x unit cost. System-scaled rows (Solar Panels, Diesel Generator(s), Generator Installation & Commissioning): system size (from Solar Design's Diesel tab) x lumpsum rate — edit that rate directly on the row."
              color={color}
            />
            <div className="mb-4">
              <FileUpload
                downloadPath="/api/financials-diesel/template"
                downloadFilename="Diesel_Cost_BOQ_Template.xlsx"
                uploadPath="/api/financials-diesel/upload"
            ...
            <p className="mt-2 text-xs text-ink-400">
              "Cables & Distribution Infrastructure" starts equal to the Solar+Battery scenario's figure (same island-wide
              infra) but is independently editable here.
            </p>
          </Card>
          ...
          <Card>
            ...
            <p className="mb-3 text-xs text-ink-400">
              Land cost is shared with Solar+Battery — the acreage/cost figures are edited on that tab; the approach used
              here is chosen by the Land Cost Approach parameter below.
            </p>
