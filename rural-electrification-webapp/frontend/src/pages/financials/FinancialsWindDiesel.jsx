...
const SYSTEM_SCALED = new Set(["Wind Turbine(s)", "Diesel Generator(s)", "Generator Installation & Commissioning"]);
const blankDash = (v) => (v === null || v === undefined || v === "" ? "—" : v);

export default function FinancialsWindDiesel({ color }) {
  const { data: defaults, error: loadError } = useApiGet("/api/financials-wind-diesel/defaults");
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      api
        .post("/api/financials-wind-diesel/compute", { boqItems, costParameters })
        .then((r) => {
          setResult(r);
          setError(null);
        })
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      const r = await api.post("/api/financials-wind-diesel/save", { boqItems, costParameters });
      setResult(r);
      setSaved(true);
      refetchProgress();
    } catch (e) {
        ...
        <>
          ...
          <Card>
            <SectionHeader
              ...
              subtitle="Costs in EUR, sourced from the workbook's Annex-VII wind economics. Standard rows: qty x unit cost. System-scaled rows (Wind Turbine(s), Diesel Generator(s), Generator Installation & Commissioning): system size (from System Design's Wind+Diesel tab) x lumpsum rate — edit that rate directly on the row."
              color={color}
            />
            <div className="mb-4">
              <FileUpload
                downloadPath="/api/financials-wind-diesel/template"
                downloadFilename="Wind_Diesel_Cost_BOQ_Template.xlsx"
                uploadPath="/api/financials-wind-diesel/upload"
            ...
            <p className="mt-2 text-xs text-ink-400">
              The generator per-kW rates start equal to the Solar+Diesel scenario's figures (generator cost doesn't depend on
              generation technology) but are independently editable here.
            </p>
          </Card>
