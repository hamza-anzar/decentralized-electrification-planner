...
def _coerce_numeric(value):
    """CSV round-tripping loses dtypes — a 'value' column mixing numbers and strings (like a place
    name) comes back as all-strings. Convert anything that parses as a number back to one."""
    try:
...
def load_defaults() -> pd.DataFrame:
    ...
    df["value"] = df["value"].apply(_coerce_numeric)
    return df
